...
class Level:
    ...
    def run(self, player_score: list[int]):
        while True:
            ...
            clock = pygame.time.Clock() # Garantindo FPS para fluidez do jogo
            while True:
                clock.tick(60)
                for ent in self.entity_list:
                    self.window.blit(source=ent.surf, dest=ent.rect) # desenhando imagem na tela
                    ent.move()
                ...
                for event in pygame.event.get():
                    ...
                    if event.type == pygame.QUIT:
                        pygame.quit()
                        sys.exit()
                    # Gerando diferentes enemies
                    if event.type == EVENT_ENEMY:
                        choice = random.choice(('Enemy1', 'Enemy2'))
                        self.entity_list.append(EntityFactory.get_entity(choice))
                    # Tempo limite para passar de fase
                ...
                # Mostra quantas entidades tem no level
