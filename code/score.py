...
from pygame.constants import K_BACKSPACE, K_SPACE
...
class Score:
    def __init__(self, window):
        self.window = window
        self.surf = pygame.image.load("./asset/ScoreBg.png").convert_alpha()  # carregando imagem
        self.rect = self.surf.get_rect(left=0, top=0)  # criando retangulo

    def save(self, game_mode: str, player_score: list[int]):
        pygame.mixer_music.load('./asset/Score.mp3')  # Definindo arquivo de musica
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.QUIT:
                    ...
                    if event.key == K_RETURN and len(name) == 4:
                        ...
                        name = name[:-1]
                    # Qualquer outra tecla contabiliza no nome que o jogador informar
                    else:
                        if len(name) < 4:
                            name += event.unicode
